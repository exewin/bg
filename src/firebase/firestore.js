...
export const currentTimeDB = async () => {
  ...
  await fetch('https://worldtimeapi.org/api/timezone/Europe/Warsaw')
    ...
    .then(data => date = new Date(data.utc_datetime)) // eslint-disable-line
...
export const addStatDB = async (uid, name) => {
  ...
  if (characterData) { await setDoc(doc(firestore, `users/${uid}`), characterData, { merge: true }) } else { return 'error, stat is not valid' }
}
...
export const equipItemDB = async (uid, itemIndex, slotToPlaceIndex = null) => {
  ...
  if (characterData) { await setDoc(doc(firestore, `users/${uid}`), characterData, { merge: true }) } else { return "item can't be equipped." }
}
...
export const unequipItemDB = async (uid, itemIndex) => {
  ...
  if (characterData) { await setDoc(doc(firestore, `users/${uid}`), characterData, { merge: true }) } else { return "item can't be unequipped." }
}
...
export const discardItemDB = async (uid, itemIndex, itemType) => {
  ...
  if (characterData) { await setDoc(doc(firestore, `users/${uid}`), characterData, { merge: true }) } else { return "item can't be discarded." }
}
