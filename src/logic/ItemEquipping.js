...
export const equipItem = async (characterData, itemIndex, slotToPlaceIndex = null) => {
  if (characterData.equipped.length < ITEM_TYPES) { fillEquipmentWillNulls(characterData) }
  ...
  if (characterData.equipped[index - 1]) { unequipItemWithNoLenghtLimit(characterData, index - 1) }
